  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <Link href="/vehicles" className="hover:text-slate-700">Our Stock</Link>
        <span>/</span>
      ...
      <div className="mt-6 flex flex-wrap items-start justify-between gap-6">
        <div>
          ...
          {vehicle.stockNumber && (
            <p className="mt-1 text-xs text-slate-500">Stock: {vehicle.stockNumber}</p>
          )}
        </div>
        <div className="text-right">
          <p className="text-xs text-slate-500">{vehicle.isDriveAway ? 'Drive Away' : 'Price'}</p>
          <p className="text-4xl font-semibold text-slate-900">${vehicle.price.toLocaleString()}</p>
          <p className="mt-1 text-xs text-slate-500">{vehicle.condition} condition</p>
        </div>
      </div>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          ...
          <div className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="text-xl font-semibold text-slate-900">Key specifications</h2>
          ...
          <div className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 grid gap-4 text-sm text-slate-600 sm:grid-cols-2">
              {vehicle.variant && <Spec label="Variant" value={vehicle.variant} />}
              {vehicle.stockNumber && <Spec label="Stock Number" value={vehicle.stockNumber} />}
              {vehicle.vin && <Spec label="VIN" value={vehicle.vin} />}
              {vehicle.registration && <Spec label="Registration" value={vehicle.registration} />}
              {vehicle.regoExpiry && <Spec label="Rego Expiry" value={vehicle.regoExpiry} />}
              {vehicle.bodyColour && <Spec label="Body Colour" value={vehicle.bodyColour} />}
              {vehicle.interiorColour && <Spec label="Interior Colour" value={vehicle.interiorColour} />}
              {vehicle.doors && <Spec label="Doors" value={`${vehicle.doors}`} />}
              {vehicle.seats && <Spec label="Seats" value={`${vehicle.seats}`} />}
              {vehicle.cylinders && <Spec label="Cylinders" value={`${vehicle.cylinders}`} />}
              {vehicle.engineSize && <Spec label="Engine Size" value={vehicle.engineSize} />}
              {vehicle.driveType && <Spec label="Drive Type" value={vehicle.driveType} />}
              {vehicle.gears && <Spec label="Gears" value={`${vehicle.gears}`} />}
              {vehicle.fuelConsumption && <Spec label="Fuel Consumption" value={vehicle.fuelConsumption} />}
              {vehicle.ancapRating && <Spec label="ANCAP Rating" value={`${vehicle.ancapRating}`} />}
              {vehicle.towingBraked && <Spec label="Towing Braked" value={`${vehicle.towingBraked} kg`} />}
            </div>
          </div>

          <div className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="text-xl font-semibold text-slate-900">Dealer comments</h2>
