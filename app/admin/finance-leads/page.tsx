...
type FinanceLeadStatus = 'new' | 'contacted' | 'in_progress' | 'approved' | 'declined' | 'closed';
...
const STATUSES: FinanceLeadStatus[] = ['new', 'contacted', 'in_progress', 'approved', 'declined', 'closed'];
...
export default function FinanceLeadsPage() {
  ...
  const fetchLeads = async () => {
    try {
      const snapshot = await getDocs(collection(db, 'financeLeads'));
      ...
      console.error('Error fetching finance leads:', error);
    } finally {
      setLoading(false);
    }
  };

  const updateStatus = async (leadId: string, status: FinanceLeadStatus) => {
    try {
      await updateDoc(doc(db, 'financeLeads', leadId), { status });
      await fetchLeads();
  ...
  const deleteLead = async (leadId: string) => {
    if (!window.confirm('Delete this finance lead?')) return;
    try {
      await deleteDoc(doc(db, 'financeLeads', leadId));
      await fetchLeads();
      setSelectedLead(null);
    } catch (error) {
      ...
      <div className="lg:col-span-1">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow p-6 text-center">
            <p className="text-gray-600">No finance leads yet</p>
          </div>
        ) : (
        ...
        {selectedLead ? (
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-2 text-sm text-gray-700">
              <p><strong>Employment:</strong> {selectedLead.employmentStatus || 'Not provided'}</p>
              <p><strong>Budget:</strong> {selectedLead.budget || 'Not provided'}</p>
              <p><strong>Vehicle:</strong> {selectedLead.vehicleOfInterest || 'Not provided'}</p>
            </div>
