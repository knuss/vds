  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const filteredData = {
        ...
        includedInPrice: formData.includedInPrice.filter(
          (item) => item.trim() !== "",
        ),
        warrantyOptions: formData.warrantyOptions.filter(
          (item) => item.trim() !== "",
        ),
      };
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <input
              ...
              onChange={handleChange}
              placeholder="ANCAP rating"
              className="w-full px-4 py-2 border border-gray-300 rounded-lg"
            />
            <input
              name="towingBraked"
              type="number"
              value={formData.towingBraked}
              onChange={handleChange}
              placeholder="Towing braked"
              className="w-full px-4 py-2 border border-gray-300 rounded-lg"
            />
          </div>
        </div>
